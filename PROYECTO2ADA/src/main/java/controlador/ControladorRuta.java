...
// Controla la pantalla de detalle de una ruta: carga datos, comentarios, valoraciones y listas
public class ControladorRuta {
    ...
    // rellena todos los campos de la vista con los datos de la ruta
    private void cargarDatos() {
        ...
        vista.setValoracion(modelo.obtenerValoracionMedia(idRuta)); // usa procedimiento almacenado
    ...
    private void comentar() {
        ...
        cargarComentarios(); // recargamos la lista para que aparezca el nuevo
    }

    private void valorar() {
        // el creador no puede valorar su propia ruta
        Object[] ruta = modelo.obtenerRuta(idRuta);
        ...
        if (ok) {
            // actualizamos la valoración media en pantalla tras insertar
    ...
    // muestra las listas del usuario para que elija en cuál guardar la ruta
    private void guardarEnLista() {
        ...
        if (elegida != null) {
            ...
            // agregarRutaALista usa un procedimiento almacenado que verifica que la lista es del usuario
            boolean ok = modelo.agregarRutaALista(idLista, idRuta, idUsuario);
