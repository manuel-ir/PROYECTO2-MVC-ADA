...
public class GestorBD {
    ...
    // agrega una ruta a una lista usando el procedimiento, verificando que la lista es del usuario
    public boolean agregarRutaALista(int idLista, int idRuta, int idUsuario) {
        try {
            CallableStatement cs = con.prepareCall("{CALL agregarRutaALista(?, ?, ?, ?)}");
            cs.setInt(1, idLista);
            cs.setInt(2, idRuta);
    ...
    // devuelve la valoracion media de una ruta usando el procedimiento almacenado
    public String obtenerValoracionMedia(int idRuta) {
        try {
            CallableStatement cs = con.prepareCall("{CALL calcularMediaValoracion(?, ?)}");
            cs.setInt(1, idRuta);
            cs.registerOutParameter(2, Types.DOUBLE);
            cs.execute();
            Object media = cs.getObject(2);
            if (media != null) return String.valueOf(cs.getDouble(2));
        } catch (SQLException e) {
    ...
    // elimina una ruta solo si el usuario es el creador
    public boolean borrarRuta(int idRuta, int idUsuario) {
        String sql = "DELETE FROM RUTA WHERE id_ruta = ? AND id_creador = ?";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, idRuta);
            ps.setInt(2, idUsuario);
    ...
    // inserta un nuevo usuario y crea su lista Favoritos en una transaccion
        ...
        try {
            ...
            if (keys.next()) {
                ...
                PreparedStatement psLista = con.prepareStatement(
                    "INSERT INTO LISTA (nombre_lista, id_usuario) VALUES ('Favoritos', ?)");
                psLista.setInt(1, idUsuario);
            ...
            try { con.setAutoCommit(true); } catch (SQLException e) { System.err.println(e.getMessage()); }
        }
    }
}
